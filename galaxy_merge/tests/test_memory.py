...
class TestCompactor:
    def test_compaction_logs_events_and_preserves_mission_state(self, tmp_path):
        ...
        atomic_write(
            session_dir / "goal.json",
            json.dumps(
                {
                    "goal": "Fix failing login page",
                    "status": "testing",
                    "parsed": {
                        "task_type": "webapp_repair",
                        "estimated_scope": "src/login",
                    },
        ...
        event_log.emit(
            "goal_received", session_id="sess_a", goal="Fix failing login page"
        )
        event_log.emit("file_changed", session_id="sess_a", path="src/login.tsx")
        event_log.emit(
            "browser_console_error", session_id="sess_a", message="ReferenceError"
        )
        event_log.emit("web_search_completed", session_id="sess_a", source="duckduckgo")
        event_log.emit(
            "council_completed", session_id="sess_a", roles=["planner", "reviewer"]
        )
        event_log.emit("fusion_completed", session_id="sess_a", changes_proposed=1)
        event_log.emit(
            "provider_failed", session_id="sess_a", provider_id="p1", model="m1"
        )
        event_log.emit(
            "location_classified", session_id="sess_a", classification="local_taskscope"
        )
        event_log.emit(
            "tool_call_blocked",
            session_id="sess_a",
        ...
        event_types = [
            event["event"] for event in EventLog(session_dir / "events.jsonl").replay()
        ]
    ...
    def test_compaction_redacts_placeholder_env_values_from_summary_and_event_logs(
        self, tmp_path
    ):
        gm_dir = tmp_path / ".gm"
        ...
        EventLog(session_dir / "events.jsonl").emit(
            "goal_received", session_id="sess_secret", goal=placeholder_env
        )
        ...
        compaction_events = [
            event
            for event in EventLog(session_dir / "events.jsonl").replay()
