...
def test_fusion_reports_full_confidence_when_all_required_perspectives_succeed() -> (
    None
):
    # Given: every core council perspective returns a valid parsed result.
    results = {
        "planner": [
            {"parsed": {"steps": ["inspect"], "completion_criteria": ["tests pass"]}}
        ],
        "implementer": [
            {"parsed": {"changes": [{"file": "app.py", "diff": "print('ok')"}]}}
        ],
        "reviewer": [
            {
                "parsed": {
...
def test_fusion_reduces_confidence_and_marks_missing_perspectives_when_roles_fail() -> (
    None
):
