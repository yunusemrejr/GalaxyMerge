const API = {
  ...
  getEvents(since) {
    const url = Number.isFinite(since) ? `/api/events?since=${Number(since)}` : '/api/events';
    return this.get(url);
  },
  ...
  getBrowserScreenshot(sessionId = 'gui') {
    return this.get(`/api/browser/screenshot?session_id=${encodeURIComponent(sessionId)}`);
  },
  postBrowserOpen(url) {
    return this.post('/api/browser/open', { url });
  },

  getWebSearch(q, source = 'duckduckgo') {
    return this.get(`/api/web/search?q=${encodeURIComponent(q)}&source=${encodeURIComponent(source)}`);
  },
  postWebFetch(url, source = 'duckduckgo') {
    return this.post('/api/web/fetch', { url, source });
  },
  getGithubScan(url) {
    return this.get(`/api/github/scan?url=${encodeURIComponent(url)}`);
  },
