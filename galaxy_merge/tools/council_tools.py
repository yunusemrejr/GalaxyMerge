...
def make_council_tools(provider_registry, fusion_config: dict[str, Any]) -> list[tuple[ToolSchema, Any]]:
    ...
    async def council_spawn(
        ...
        if council_name:
            ...
            if not named:
                return ToolResult(success=False, error=f"council '{council_name}' not found in config")
            config = named
        if roles:
            config["roles"] = roles
    ...
    return [
        (ToolSchema("council.spawn", "Spawn a council of AI roles to work on a goal in parallel", parameters={
            "goal": {"type": "string", "required": True},
            "council_name": {"type": "string", "default": None},
            "roles": {"type": "object", "default": None},
        }), council_spawn),
        (ToolSchema("council.synthesize", "Synthesize council results into fused output", parameters={
            "council_results": {"type": "object", "required": True},
            "synthesis_mode": {"type": "string", "default": "evidence_weighted"},
        }), council_synthesize),
        (ToolSchema("council.review", "Review a fusion result for quality and safety", parameters={
