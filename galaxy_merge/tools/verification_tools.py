    ...
    async def verify_syntax(path: str) -> ToolResult:
        ...
        if ext == ".py":
            ...
            if result.returncode == 0:
                return ToolResult(
                    success=True, data={"valid": True, "message": "Python syntax OK"}
                )
            else:
                return ToolResult(
                    success=True, data={"valid": False, "error": result.stderr}
                )

        return ToolResult(
            success=True, data={"valid": True, "message": "no syntax check available"}
        )
