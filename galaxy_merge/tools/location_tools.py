...
def make_location_tools(
    workroot: Path, gm_dir: Path, install_dir: Path | None = None
    ...
    return [
        (
            ToolSchema(
                "location.classify",
                "Classify a path or command into a location class",
                parameters={
        ...
        (
            ToolSchema("location.registry.read", "Read the current location registry"),
            location_registry_read,
        ),
    ]
