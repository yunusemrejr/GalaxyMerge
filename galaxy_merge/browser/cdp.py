...
def wait_for_page_ws_url(port: int, timeout: float = 8, stop: threading.Event | None = None) -> str:
...
def send_page_command(port: int, method: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    def _handle_event(self, event: dict[str, Any]) -> None:
        ...
        if method == "Runtime.consoleAPICalled":
            ...
            if self.page_errors and entry.get("level") == "error":
                self.page_errors.add_error("Log.entryAdded", entry.get("text", ""), entry.get("source", "log"))
            ...
            if self.page_errors and status >= 400:
                self.page_errors.add_error("Network.responseReceived", f"HTTP {status}", response.get("url", ""))
            ...
            if self.page_errors:
                self.page_errors.add_error("Network.loadingFailed", params.get("errorText", ""), params.get("requestId", ""))
        elif method == "Runtime.exceptionThrown":
            details = params.get("exceptionDetails", {})
            exception = details.get("exception", {})
            message = exception.get("description") or exception.get("value") or details.get("text", "")
            self.console.add_error(str(message), "runtime")
            if self.page_errors:
                self.page_errors.add_error("Runtime.exceptionThrown", str(message), details.get("url", "runtime"))
