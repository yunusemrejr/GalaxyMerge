...
class PathPolicy:
    ...
    def _check_git_hooks(self, resolved_str: str) -> dict[str, Any] | None:
        for pattern in GIT_HOOK_PATTERNS:
            if pattern in resolved_str:
                return {"decision": "block", "reason": f"write to git hooks/config blocked: {pattern}"}
        return None
    ...
    def _check_credential_path(self, resolved_str: str) -> dict[str, Any] | None:
        ...
        for part in CREDENTIAL_CHECK_PATTERNS:
            if part in path_lower:
                return {"decision": "block", "reason": f"credential file path blocked: {part}"}
        return None
    ...
    def check_write(self, path: Path) -> dict[str, Any]:
        ...
        for blocked in BLOCKED_WRITE_PATHS:
            if is_relative_to(resolved, blocked):
                return {"decision": "block", "reason": f"write to system path blocked: {blocked}"}

        if is_relative_to(resolved, self.home):
            relative = str(resolved.relative_to(self.home))
            for pattern in BLOCKED_USER_PATTERNS:
                if relative == pattern or relative.startswith(pattern + "/") or f"/{pattern}" in relative:
                    return {"decision": "block", "reason": f"write to protected user path blocked: {pattern}"}
        ...
        if not is_relative_to(resolved, self.workroot):
            return {"decision": "block", "reason": f"write outside WorkRoot blocked: {path}"}

        if resolved != path:
            return {"decision": "allow_with_audit", "reason": "write via symlink, real path verified safe"}
