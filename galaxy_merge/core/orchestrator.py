...
class Orchestrator:
    def __init__(
        self, session: Session, config_dir: Path, install_dir: Path | None = None
    ):
        ...
        self.safety_audit = SafetyAudit(
            session.gm_dir / "safety" / "blocked_actions.jsonl"
        )
        self.safety = SafetyGovernor(
            session.workroot, session.gm_dir, self.safety_audit
        )
        self.sandbox = Sandbox(session.workroot)
        self.tool_kernel = ToolKernel(self.safety, self.event_log)
        self.providers = ProviderRegistry(
            config_dir, event_log=self.event_log, session_id=session.session_id
        )
        ...
        self.location_classifier = LocationClassifier(
            session.workroot, session.gm_dir, install_dir
        )
    ...
    async def initialize(self) -> None:
        ...
        self.location_registry.init_from_project(
            self.session.workroot, self.session.gm_dir
        )
    ...
    def _register_tools(self) -> None:
        ...
        for schema, handler in make_shell_tools(
        ...
        for schema, handler in make_location_tools(
            self.session.workroot, self.session.gm_dir, self.install_dir
        ):
            self.tool_kernel.register(schema, handler)

        from galaxy_merge.tools.security_tools import make_security_tools

        for schema, handler in make_security_tools(
            self.session.workroot, self.install_dir
        ):
            self.tool_kernel.register(schema, handler)

        from galaxy_merge.tools.browser_tools import make_browser_tools

        for schema, handler in make_browser_tools(
    ...
    async def execute_goal(self, goal: str) -> dict[str, Any]:
        ...
        if memory_context.get("notes"):
            self.event_log.emit(
                "note_loaded",
                session_id=self.session.session_id,
                notes_count=len(memory_context.get("notes", "")),
            )
        if memory_context.get("injected_notes"):
            self.event_log.emit(
                "note_loaded",
                session_id=self.session.session_id,
                injected=memory_context["injected_notes"],
            )
        analyze_workroot(self.session.workroot)
        ...
        if matched_skills:
            self.event_log.emit(
                "skill_selected",
                session_id=self.session.session_id,
                skills=[s["name"] for s in matched_skills[:3]],
            )
        ...
        self.event_log.emit(
            "fusion_completed",
            session_id=self.session.session_id,
            changes_proposed=fused.get("changes_proposed", 0),
        )
        ...
        for change in fused.get("plan", []):
            ...
            if tool_name:
                loc = self.location_classifier.classify(
                    params.get("path", tool_name), "path"
                )
                ...
                result = await self.tool_kernel.execute(
                    tool_name, params, self.session.session_id
                )
                self.event_log.emit(
                    ...
                    duration_ms=result.data.get("_duration_ms", 0)
                    if result.data
                    else 0,
                )
        ...
        self.event_log.emit(
            "completion_review_started", session_id=self.session.session_id
        )
        if complete:
            ...
            self.event_log.emit(
                "completion_rejected", session_id=self.session.session_id
            )
    ...
    def _promote_to_memory(
        self, goal: str, fused: dict[str, Any], verification: dict[str, Any]
    ) -> None:
        if fused.get("changes_proposed", 0) > 0:
            ...
            if files:
                self.project_memory.record_fact(
                    f"changed files: {', '.join(sorted(files))}", source="session"
                )
        if verification.get("passed"):
            test_commands = [
                c for c in fused.get("summary", "").split() if "test" in c.lower()
            ]
            if test_commands:
                self.project_memory.record_lesson(
                    f"verified with: {' '.join(test_commands)}", category="testing"
                )
        self.project_memory.record_fact(
    ...
    async def _verify(self, fused: dict[str, Any]) -> dict[str, Any]:
        ...
        if fused.get("schema_errors"):
            issues.extend(
                f"schema error: {error}" for error in fused.get("schema_errors", [])
            )
        for change in fused.get("plan", []):
